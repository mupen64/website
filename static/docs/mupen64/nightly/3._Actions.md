...
This page contains a breakdown of all menu actions along with their
functionality and is hierarchically structured like the in-app menus.
...
If the ROM is invalid or any other errors preventing an emulation start happen,
...
If tasks such as movie playback, recording, or video capture are running, you
will be prompted prior to confirm the close action.
...
The list can be frozen, which prevents it being changed, or reset, which clears
all items.
...
The emulator will remain paused after the frame advance operation completes and
can be unpaused manually.
...
The current slot can be seen in the last segment of the statusbar and ranges
from 1 to 10.
...
If a movie is being recorded or played back, loading a state from created in
another movie (or none at all) might cause a desync and requires user
confirmation.
...
Contains a list of the most recently played or recorded movies, with the latest
ones at the top.
...
If enabled, the emulator will pause at the last frame of a movie during playback
and not allow unpausing until you either load a savestate from a different frame
or reset the ROM.
...
until the operation is finished. Closing the seeker will stop any ongoing seek operation.
...
Shows the address of the RDRAM buffer and allows copying an auto-generated
Config.xml entry for [STROOP](https://github.com/SM64-TAS-ABC/STROOP).
