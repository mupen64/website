...
It keeps the same version number for compatibility reasons, but defines new data in the data gaps of the old format. An extended version of `0` indicates an old movie with no extended data. The latest extended version is `4`.
...
| - | - | - |
| 0 | WiiVC | Whether the movie was recorded with WiiVC emulation mode enabled. |
| 1 | c.eq.s accurate | Whether the movie was recorded with an accurate implementation of the `c.eq.s` instruction where `(NaN == any) == false` instead of `(NaN == any) == true` (legacy behavior). |
| Rest | Reserved | Must be 0, or a warning will be shown on playback. |
...
| 0 | WiiVC | Whether the movie was recorded with WiiVC emulation mode enabled. |
| 1 | c.eq.s accurate | Whether the movie was recorded with an accurate implementation of the `c.eq.s` instruction where `(NaN == any) == false` instead of `(NaN == any) == true` (legacy behavior). |
| 2 | Accurate RDP completion | Whether the movie was recorded with RDP task completion signalled after RSP task completion, instead of both being signalled at the same instant (legacy behavior). |
| Rest | Reserved | Must be 0, or a warning will be shown on playback. |

### Extended Data

The extended data block is 32 bytes long and begins at offset `0x024` in the movie header.

*Versions 1–3*

| Offset | Type | Name | Additional Info |
| - | - | - | - |
| `0x024` | uint8_t[4] | Authorship tag | Identifies the program or tool which created the movie. Mupen64 writes `0x4D55504E` (`MUPN`). |
| `0x028` | uint32_t | Bruteforce extra data | Additional data regarding bruteforcing. |
| `0x02C` | uint32_t | Rerecord count (High) | The upper dword of the number of savestates loaded while recording the movie. The complete count is `(High << 32) \| Low`. |
| Rest | - | Reserved | Must be zero-filled. |
...
| `0x028` | uint32_t | Bruteforce extra data | Additional data regarding bruteforcing. |
| `0x02C` | uint32_t | Rerecord count (High) | The upper dword of the number of savestates loaded while recording the movie. The complete count is `(High << 32) \| Low`. |
| `0x030` | double | CPU counter factor | The CPU counter factor used during recording. |
| `0x038` | double | RCP lag factor | The factor by which RCP lag was multiplied during recording. |
| Rest | - | Reserved | Must be zero-filled. |

The extended data block is only valid when the extended version is nonzero.

# ST

Mupen64 will create savestates compressed using [lz4](https://en.wikipedia.org/wiki/LZ4_(compression_algorithm)) by default. This can be switched to [Gzip](https://en.wikipedia.org/wiki/Gzip) in the settings ("Core > Fast Savestates").

Savestates that are uncompressed, gzip-compressed, or lz4-compressed are automatically detected and loaded seamlessly.
